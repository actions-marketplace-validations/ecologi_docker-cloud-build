...
import { CloudBuildOptions } from "../gcp/CloudBuildClient";

declare type DescriptionSize = "large" | "medium" | "small" | "tiny";
const DESCRIPTION_SIZES = ["large", "medium", "small", "tiny"];

declare type GcrRegion = "gcr.io" | "eu.gcr.io" | "us.gcr.io" | "asia.gcr.io";
const GCR_REGIONS = ["gcr.io", "eu.gcr.io", "us.gcr.io", "asia.gcr.io"];
...
const invalidEnumArgument = (argument: string, expected: string[], actual: string): string => {
  return (
    `Invalid value passed for argument ${argument}. ` + `Expected one of ${expected.join(", ")}, but got ${actual}.`
  );
};

const invalidStringArgument = (argument: string, expected: string, actual: string): string => {
  return `Invalid value passed for argument ${argument}. ` + `Expected ${expected}, but got ${actual}.`;
};
...
export const parseInput = (): InputParams => {
  ...
  if (DESCRIPTION_SIZES.indexOf(githubCommitStatusDescription) === -1) {
    throwLoadInputError(
      invalidEnumArgument("github-commit-status-description", DESCRIPTION_SIZES, githubCommitStatusDescription)
    );
  }
